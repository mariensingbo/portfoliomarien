...
export default class Worknav {
  constructor(options) {
    ...
    let raf = function(){
      ...
      if (attractMode) {
        ...
        this.setZIndex(this.activeMeshIndex);
 ...
 setZIndex(activeMeshIndex) {
  // Get all elements with the class '.n'
  let elements = document.querySelectorAll('.n');

  // Set z-index of all elements to 1000
  for(let i = 0; i < elements.length; i++) {
      elements[i].style.zIndex = 1000;
  }

  // Set z-index of the active mesh to 1001
  document.querySelector('.n' + activeMeshIndex).style.zIndex = 1001;
}
...
dispose() {
  ...
  if(nav) {
    ...
    nav.removeEventListener('mouseleave', this.onMouseLeave);
  }
